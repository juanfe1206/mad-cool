...
class Person(Festival):
  ...
  def behaviour(self, festival, bathrooms_list, bars_list, food_stands_list, merch_stands_list, stages_list):
    ...
    while True:
      ...
      self.hunger += random.choices([0, 1, 2], weights=[0.3, 0.35, 0.35])[0]
      self.thirst += random.choices([0, 1, 2], weights=[0.5, 0.3, 0.2])[0]
      self.need_bathroom += random.choices([0, 1, 2], weights=[0.7, 0.2, 0.1])[0]
      self.want_merch += random.choices([0, 1, 2], weights=[0.8, 0.15, 0.05])[0]
  ...
  def go_drink(self):
    self.thirst = 0
    #print(f'Person {self.id} is drinking')
